...
async function deletePreviousUploads (uploads, prefixUploadName, numberToKeep) {
  ...
  for await (const upload of uploads) {
    if (upload?.name.startsWith(prefixUploadName)) {
      ...
      console.log(`  Preserving (excluded) ${upload.name} CID: ${upload.cid}`)
    }
  }
  ...
  console.log(`Keeping maximum of ${numberToKeep} entries`)
  console.log(`Would delete ${toDelete.length} entries (disabled)`)
